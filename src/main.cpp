...
Font Init_Load_Font_Ex()
{
    ...
    for (int i = 48; i <= 57; ++i)
        codepoints[count++] = i; // Цифры 0-9
    for (int i = 65; i <= 90; ++i)
        codepoints[count++] = i; // A-Z
    for (int i = 97; i <= 122; ++i)
        codepoints[count++] = i; // a-z
    for (int i = 1040; i <= 1103; ++i)
        codepoints[count++] = i; // А-я
...
int main()
{
    ...
    while (!WindowShouldClose() )
    {
        ...
        if (isDragging)
        {
            ...
            SetWindowPosition(windowPos.x + currentMousePos.x - dragOffset.x, windowPos.y + currentMousePos.y - dragOffset.y);
        }
